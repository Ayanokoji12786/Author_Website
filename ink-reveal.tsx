...
export default function InkReveal({
  ...
  // Destructure array props into primitives so useCallback dependencies
  // stay stable across re-renders (array refs change every render)
  const [mc0, mc1, mc2] = maskColor;
  const [w0, w1, w2] = wobble;
  const [gs0, gs1, gs2] = gradientStops;
  ...
  const carveInk = useCallback(
    (
      ...
      g.addColorStop(0, `rgba(0,0,0,${gs0 * alpha})`);
      g.addColorStop(0.5, `rgba(0,0,0,${gs1 * alpha})`);
      g.addColorStop(1, `rgba(0,0,0,${gs2 * alpha})`);
      ctx.fillStyle = g;
      ctx.beginPath();
      for (let i = 0; i <= segments; i++) {
        ...
        const wob =
          0.78 +
          w0 * Math.sin(a * 3 + seed) +
          w1 * Math.sin(a * 5 + seed * 2.1) +
          w2 * Math.sin(a * 7 + seed * 0.7);
    ...
    [segments, w0, w1, w2, gradientInnerRadius, gs0, gs1, gs2]
  );
  ...
  }, [carveInk, mc0, mc1, mc2, lifetime, rStart]);
